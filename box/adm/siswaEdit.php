...
if(isset($_POST["submit"])){
  ...
  $fileimg = $_FILES["fileimg"]["name"];
      ...
      $x = explode('.', $fileimg);
      $ekstensi = strtolower(end($x));
      $ukuran = $_FILES['fileimg']['size'];
      $img_tmp = $_FILES['fileimg']['tmp_name'];  
      if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
        if($ukuran < 500000){
         $fileimgBaru = uniqid();
          $fileimgBaru .= '.';
          $fileimgBaru .= $ekstensi;
         $move = move_uploaded_file($img_tmp, '../../img/siswa/'.$fileimgBaru);
          if($move){
            if (unlink('../../img/siswa/'.$imglama))
            { "Deleted ../../img/siswa/$imglama";
              "<META HTTP-EQUIV=Refresh CONTENT='1; URL=tabel.php'>";
            } $img =$fileimgBaru;
          }else{echo"<script>
                      alert(' Gagal uploud img');
                      document.location.href = 'tabel.php';
                      </script>"; return false;}
        }else{echo"<script>
                    alert('Gagal uploud img ... Ukuran img max 500 kb');
                    document.location.href = 'tabel.php'; 
                    </script>";return false;}
      }else{echo"<script>
                  alert('Gagal uploud img ... Ekstensi img yang diperbolehkan png dan jpg');
                  document.location.href = 'tabel.php';
                  </script>";return false;}
      } 
...
<div class="row">
  ...
  <div class="col-md-6">
    <div class="form-group">
           <label for="fileimg" class="control-label">Foto Lama : </label><img src="../../img/siswa/<?=$row["img"]; ?>" width = "50px">
           <input type="file" name="fileimg" value="" class="form-control" id="fileimg">
            <input type="hidden" name="imglama" value="<?=$row["img"]; ?>">
    </div>
  </div>
