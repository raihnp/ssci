...
function smtp_mail($to, $subject, $message, $from_name, $from, $cc, $bcc, $debug=false) {
  ...
  $mail->SMTPAuth 	= true;                        				
  $mail->Host 		  = 'cobassci.my.id;203.175.8.45';  // Masukkan Server SMTP
  $mail->Port 		  = 587;                                      // Masukkan Port SMTP
  $mail->SMTPSecure = 'tls';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
  $mail->Username 	= 'slamet@cobassci.my.id';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
  $mail->Password 	= 'fe3+sfe3+';        						          // Masukkan Password dari Email tsb
  $default_email_from       = 'slamet@cobassci.my.id';        // Masukkan default from pada email
  $default_email_from_name  = 'feri@nto';           // Masukkan default nama dari from pada email
